...
final class logicsTest extends \PHPUnit_Framework_TestCase {
    ...
    public function setUp() {
        ...
        $oneSubLogic = getLogicsData($functionData, 0);

        $this->baseRoute = '/';
        $this->softwareNameInDatastore = 'apis_irestful';
        $this->newSoftwareConfigUrl = 'http://code.irestful.com/configs/softwares/apis/logics.json';
        ...
        $this->containerData = getLogicsContainerData();
        $this->data = array(
            'first' => getLogicsData($functionData, 0, array($paramsData), array($oneSubLogic)),
            'second' => getLogicsData($functionData, 1, array($paramsData), array($oneSubLogic))
        );
    ...
    public function testSingle_Success() {
        crud($this, $this->baseRoute, $this->dependencies, $this->softwareNameInDatastore, $this->newSoftwareConfigUrl, $this->containerData, $this->data, $this->preAdditionalData, $this->postAdditionalContainerData, array('params', 'sub_logics'));
    }
