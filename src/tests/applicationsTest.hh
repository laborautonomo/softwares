...
final class applicationsTest extends \PHPUnit_Framework_TestCase {
    ...
    public function setUp() {
        ...
        $routeData = getRouteData(0, $cliRouteData);

        $parentApplicationData = getApplicationData(0);

        $this->baseRoute = '/';
        $this->softwareNameInDatastore = 'apis_irestful';
        $this->newSoftwareConfigUrl = 'http://code.irestful.com/configs/softwares/apis/applications.json';
        $this->dependencies = array(
            ...
            'http://code.irestful.com/configs/softwares/apis/routes.json' => array(
                'base_route' => '/',
                'container_data' => getRoutesContainerData(),
                'data' => $routeData
            )
        );

        $this->containerData = getApplicationContainerData();
        $this->data = array(
            'first' => getApplicationData(1, array($routeData), array($parentApplicationData)),
            'second' => getApplicationData(2, array($routeData), array($parentApplicationData))
        );
        ...
        $this->postAdditionalContainerData = array(
            getApplicationsRoutes(),
            getApplicationsParentApplications()
        );
    }
    ...
    public function testSingle_Success() {
        crud($this, $this->baseRoute, $this->dependencies, $this->softwareNameInDatastore, $this->newSoftwareConfigUrl, $this->containerData, $this->data, $this->preAdditionalData, $this->postAdditionalContainerData, array('routes', 'parent_applications'));
    }
